...
public class clientGet {
	...
	public static void main(String[] args) throws IOException {
		...
		*/
		...
		read_filePath = "U:" + "\\" + "urlOfDictionary.txt";
		write_filePath = "U:" + "\\" + "sources.txt";
					...
					try {
						...
						for (int j = (numberOfLines - 1); j >= 0; j--)
						{
							...
							for (int k = 0; k < (urlLineArray[j].length() - scanLimit); k++)
							{
								...
								if (urlLineArray[j].trim().startsWith("<b>Source URLs:"))
								{
									ulCount++;
									indexOfClients = j;
						...
						int a = 0;
						String processorName = urlLineArray[1176].trim();
						processorName = processorName.substring(48, (processorName.length()-1));
